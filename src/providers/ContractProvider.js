...
export const ContractProvider = ({ children }) => {
  ...
  const fromWeiEx = (wei, unit = "ether") =>
    (parseFloat(Web3.utils.fromWei(wei, unit)) * 93 / 95).toFixed(3);
  const toWei = (amount, unit = "ether") => Web3.utils.toWei(amount, unit);

  return (
    <ContractContext.Provider
      value={{ web3, busdcontract, contract, wrongNetwork, getBusdBalance, fromWei, toWei, getBusdApproved, fromWeiEx }}
    >
      {children}
    </ContractContext.Provider>
  );
